...
namespace fast_planner
{
  ...
  void HeadingPlanner::setMap(const shared_ptr<MapServer> &map)
  {
    map_server_ = map;
  }
    ...
    for (int x = lbi[0]; x <= ubi[0]; ++x)
      for (int y = lbi[1]; y <= ubi[1]; ++y)
        for (int z = lbi[2]; z <= ubi[2]; ++z)
        {
          ...
          pt_idx << x, y, z;
          if (!(map_server_->getOccupancy(pt_idx) == voxel_mapping::OccupancyType::UNKNOWN))
            continue;
          if (!map_server_->isInBox(pt_idx))
            continue;
          map_server_->getOccupancyGrid()->indexToPosition(pt_idx, check_pt);
          ...
          { // unvisited cell, should raycast
            ...
            while (casters_[task_id]->step(ray_pt))
            {
              ...
              if (map_server_->getOccupancy(ray_id) == voxel_mapping::OccupancyType::OCCUPIED)
              {
                result = 2;
                break;
              }
  ...
  void HeadingPlanner::initCastFlag(const Eigen::Vector3d &pos)
  {
    ...
    Eigen::Vector3i lbi, ubi;
    map_server_->getOccupancyGrid()->positionToIndex(pos - vec, lbi);
    map_server_->getOccupancyGrid()->positionToIndex(pos + vec, ubi);
                                   ...
                                   Eigen::Vector3i &lb, Eigen::Vector3i &ub)
  {
    ...
    map_server_->getOccupancyGrid()->boundBox(lbd, ubd);
    // visualizeBox(lbd, ubd);

    map_server_->getOccupancyGrid()->positionToIndex(lbd, lb);
    map_server_->getOccupancyGrid()->positionToIndex(ubd, ub);
  }
  ...
  void HeadingPlanner::distToPathAndCurPos(const Eigen::Vector3d &check_pt,
                                           const Eigen::MatrixXd &ctrl_pts, std::pair<double, double> &dists,
                                           bool debug)
  {
