...
void HovThrKF::simple_update(Eigen::Quaterniond q, double u, Eigen::Vector3d acc)
{
  ...
  Vector3d acc_des = Vector3d(0, 0, u * param.full_thrust / param.mass) - bRw * Vector3d(0, 0, param.gra);
  double compensate = (acc_des(2) - acc_body2_filter) * 0.001;
  x(0) = x(0) + compensate;

  // std::cout << "acc_des=" << acc_des.transpose()  << " acc_body2_filter=" <<
  // acc_body2_filter << std::endl;
