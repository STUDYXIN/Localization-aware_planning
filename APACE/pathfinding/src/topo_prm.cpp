...
namespace fast_planner
{
  ...
  void TopologyPRM::init(ros::NodeHandle &nh)
  {
    ...
    resolution_ = edt_environment_->map_server_->getResolution();
    ...
    for (int i = 0; i < (int)max_raw_path_; ++i)
    {
  ...
  bool TopologyPRM::needConnection(GraphNode::Ptr g1, GraphNode::Ptr g2, Eigen::Vector3d pt)
  {
    ...
    for (int i = 0; i < (int)g1->neighbors_.size(); ++i)
    {
      for (int j = 0; j < (int)g2->neighbors_.size(); ++j)
      {
                              ...
                              Eigen::Vector3d &pc, int caster_id)
  {
    ...
    while (casters_[caster_id]->step(ray_pt))
    {
      ...
      dist = edt_environment_->map_server_->getESDF()->getDistance(pt_id);
      if (dist <= thresh)
      {
        edt_environment_->map_server_->getESDF()->indexToPosition(pt_id, pc);
        return false;
      }
    }
    return true;
  ...
  vector<vector<Eigen::Vector3d>> TopologyPRM::pruneEquivalent(vector<vector<Eigen::Vector3d>> &paths)
  {
    ...
    for (int i = 1; i < (int)paths.size(); ++i)
    {
      ...
      for (int j = 0; j < (int)exist_paths_id.size(); ++j)
      {
    ...
    // save pruned paths
    for (int i = 0; i < (int)exist_paths_id.size(); ++i)
    {
                                                                ...
                                                                int step)
  {
    ...
    for (int i = 0; i < (int)reserve_num_ && paths.size() > 0; ++i)
    {
    ...
    for (int i = 0; i < (int)short_paths.size(); ++i)
    {
  ...
  vector<Eigen::Vector3d> TopologyPRM::discretizePath(const vector<Eigen::Vector3d> &path, int pt_num)
  {
    ...
    for (int i = 0; i < (int)pt_num; ++i)
    {
      ...
      int idx = -1;
      for (int j = 0; j < (int)len_list.size() - 1; ++j)
      {
  ...
  void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num)
  {
    ...
    for (int k = 0; k < iter_num; ++k)
    {
      ...
      for (int i = 1; i < (int)dis_path.size(); ++i)
      {
        if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt, path_id))
          continue;

        edt_environment_->evaluateEDTWithGrad(colli_pt, dist, grad);
        if (grad.norm() > 1e-3)
        {
          grad.normalize();
  ...
  void TopologyPRM::shortcutPaths()
  {
    ...
    if (parallel_shortcut_)
    {
      ...
      for (int i = 0; i < (int)raw_paths_.size(); ++i)
      {
  ...
  vector<vector<Eigen::Vector3d>> TopologyPRM::searchPaths()
  {
    ...
    for (int i = min_node_num; i <= max_node_num; ++i)
    {
      ...
      for (int j = 0; j < (int)path_list[i].size(); ++j)
      {
        filter_raw_paths.push_back(raw_paths_[path_list[i][j]]);
        if ((int)filter_raw_paths.size() >= max_raw_path2_)
        {
          reach_max = true;
          break;
        }
  ...
  void TopologyPRM::depthFirstSearch(vector<GraphNode::Ptr> &vis)
  {
    ...
    for (int i = 0; i < (int)cur->neighbors_.size(); ++i)
    {
      // check reach goal
      if (cur->neighbors_[i]->id_ == 1)
      {
        ...
        if ((int)raw_paths_.size() >= max_raw_path_)
          return;
