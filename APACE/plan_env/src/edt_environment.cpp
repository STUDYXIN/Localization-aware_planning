...
namespace fast_planner
{
    ...
    for (int x = 0; x < 2; x++)
      for (int y = 0; y < 2; y++)
        for (int z = 0; z < 2; z++)
        {
          ...
          dists[x][y][z] = map_server_->getESDF()->getDistance(map_server_->getESDF()->positionToIndex(pts[x][y][z]));
        }
  }
  ...
  void EDTEnvironment::evaluateEDTWithGrad(const Eigen::Vector3d &pos, double &dist, Eigen::Vector3d &grad)
  {
