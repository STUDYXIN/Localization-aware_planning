...
namespace fast_planner
{
    ...
    for (int i = 0; i < num_layers; ++i)
    {
      ...
      else
      {
        ...
        for (int j = 0; j < vert_num; j++)
          yaw_samples.push_back((j - half_vert_num_) * yaw_diff_);
      ...
      for (const auto &v1 : last_layer)
      {
        for (const auto &v2 : layer)
          yaw_graph.addEdge(v1->id_, v2->id_, calcCoVisibility(v1, v2));
      }
      last_layer.clear();
  ...
  double YawInitialPlanner::calcCoVisibility(YawVertex::Ptr v1, YawVertex::Ptr v2)
  {
    ...
    return calcCoVisibility(v1->pos_, v2->pos_, v1->acc_, v2->acc_, v1->yaw_, v2->yaw_, features);
  }
  ...
  double YawInitialPlanner::calcVisibility(const Eigen::Vector3d &pos, const Eigen::Vector3d &acc, const double &yaw_rad, const Eigen::Vector3d &f)
  {
