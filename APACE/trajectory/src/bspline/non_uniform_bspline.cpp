...
namespace fast_planner
{
  ...
  void NonUniformBspline::getBoundaryStates(const int &ks, const int &ke, vector<Eigen::Vector3d> &start, vector<Eigen::Vector3d> &end)
  {
  ...
  double NonUniformBspline::getKnotSpan() { return knot_span_; }
                                                ...
                                                const int &degree, Eigen::MatrixXd &ctrl_pts)
  {
    ...
    if (degree == 3)
    {
      ...
      Eigen::Vector3d pt_to_pos = Eigen::Vector3d(1, 4, 1) / 6;
      Eigen::Vector3d pt_to_vel = Eigen::Vector3d(-1, 0, 1) / (2 * ts);
      Eigen::Vector3d pt_to_acc = Eigen::Vector3d(1, -2, 1) / (ts * ts);
  ...
  double NonUniformBspline::getTimeSum() { return u_(m_ - p_) - u_(p_); }
