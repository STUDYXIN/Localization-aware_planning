...
namespace fast_planner
{
  ...
  {
    edt_env_ = edt;
    resolution_ = edt_env_->map_server_->getResolution();
    Eigen::Vector3d origin, size;
    edt_env_->map_server_->getRegion(origin, size);
                                                  ...
                                                  const Eigen::Vector3d &offset, const double &res)
  {
    ...
    while (caster_->step(qk))
    {
      ...
      dist = edt_env_->map_server_->getESDF()->getDistance(pt_id);
    ...
    edt_env_->map_server_->getESDF()->indexToPosition(min_id, min_pt);
    return min_pt;
  }
                                                        ...
                                                        int &state, Eigen::Vector3d &block)
  {
    ...
    while (caster_->step(tmp))
    {
      ...
      if (dist < min_dist)
      {
        edt_env_->map_server_->getESDF()->indexToPosition(pt_id, tmp);
        if (dist < 1e-3)
        {
          min_dist = dist;
          min_pt = tmp;
        ...
        else if (dist < 1.2)
        {
          ...
          edt_env_->evaluateEDTWithGrad(tmp, dist, grad);
          if (grad.norm() > 1e-3)
          {
                                 ...
                                 Eigen::Vector3d &pc)
  {
    ...
    while (caster_->step(ray_pt))
    {
      ...
      edt_env_->map_server_->getESDF()->indexToPosition(pt_id, pt);
      edt_env_->evaluateEDTWithGrad(pt, dist, grad);
      if (dist <= min_visib_)
      {
        edt_env_->map_server_->getESDF()->indexToPosition(pt_id, pc);
        return false;
      }
    }
    return true;
  ...
  void VisibilityUtil::findVisibPairs(const vector<Eigen::Vector3d> &pts, vector<VisiblePair> &pairs)
  {
    ...
    for (int i = 0; i < pts.size() - visible_num_; ++i)
    {
      ...
      Eigen::Vector3d grad;
      double dist;
      edt_env_->evaluateEDTWithGrad(qb, dist, grad);
      if (grad.norm() > 1e-3)
      {
        grad.normalize();
  ...
  bool VisibilityUtil::findUnknownPoint(NonUniformBspline &traj, Eigen::Vector3d &point, double &time)
  {
    ...
    if (edt_env_->map_server_->getOccupancy(pt) == voxel_mapping::OccupancyType::UNKNOWN)
                                     ...
                                     Eigen::Vector3d &min_pt)
  {
    ...
    for (double n = 0.0; n <= dist1 + 1e-3; n += 0.1)
    {
      ...
      if (edt_env_->map_server_->getESDF()->getDistance(pv) <= resolution_)
      {
                                                                  ...
                                                                  bool debug)
  {
    ...
    int unknwon_num = 0;
    double res = edt_env_->map_server_->getResolution();
    Eigen::Vector3d origin, size;
    edt_env_->map_server_->getRegion(origin, size);
    ...
    for (int j = ctrl_pts.size() - 1; j >= visible_num_; --j)
    {
      ...
      double d = edt_env_->map_server_->getESDF()->getDistance(qj);
      if (d < 9999)
      {
        if (unknwon_num < 5)
        {
