...
bool Transformer::lookupTransform(const ros::Time &timestamp, Transformation &transform)
{
  ...
  {
    if (verbose_)
      ROS_WARN_STREAM_THROTTLE(1, "[Transformer] No match found for transform timestamp: " << timestamp << " as transform queue is empty.");
    return false;
  }
...
{
  if (verbose_)
    ROS_INFO_THROTTLE(1.0, "[Transformer] Received transform, transformation queue size: %d", static_cast<int>(transform_queue_.size()));
