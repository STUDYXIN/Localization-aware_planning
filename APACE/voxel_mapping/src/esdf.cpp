...
namespace voxel_mapping
{
  ...
  void ESDF::updateLocalESDF(const VoxelIndex &idx_min, const VoxelIndex &idx_max)
  {
    for (int x = idx_min[0]; x <= idx_max[0]; x++)
      for (int y = idx_min[1]; y <= idx_max[1]; y++)
      {
        fillESDF(
            ...
            [&](int z, double val)
            { tmp_buffer1_[indexToAddress(VoxelIndex(x, y, z))].value = val; },
    ...
    for (int x = idx_min[0]; x <= idx_max[0]; x++)
      for (int z = idx_min[2]; z <= idx_max[2]; z++)
      {
        fillESDF(
            [&](int y)
            { return tmp_buffer1_[indexToAddress(VoxelIndex(x, y, z))].value; },
            [&](int y, double val)
  ...
  void ESDF::getDistanceAndGradient(const Position &pos, double &distance, Eigen::Vector3d &gradient)
  {
    CHECK(isInMap(pos));

    /// 拿到包围pos的正方体8个顶点储存在ESDF中的距离值
    /// 通过三次线性插值计算pos处的distance值
    Eigen::Vector3d pos_m = pos - 0.5 * map_config_.resolution_ * Eigen::Vector3d::Ones();
    Eigen::Vector3i idx = positionToIndex(pos_m);
    Eigen::Vector3d idx_pos = indexToPosition(idx);
    Eigen::Vector3d diff = (pos - idx_pos) * map_config_.resolution_inv_;
  ...
  void ESDF::getDistanceAndGradient(const VoxelIndex &idx, double &distance, Eigen::Vector3d &gradient)
  {
    CHECK(isInMap(idx));
