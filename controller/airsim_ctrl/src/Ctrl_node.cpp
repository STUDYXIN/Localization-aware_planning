...
int main(int argc, char *argv[])
{
  ...
  ROS_INFO("Initial value for hov_thr set to %.2f/%.2f", fsm.hov_thr_kf.get_hov_thr(), param.mass * param.gra / param.full_thrust);
  ROS_INFO("Hovering thrust kalman filter is %s.", param.hover.use_hov_percent_kf ? "used" : "NOT used");

  fsm.controller.config();

  // The odom sub topic is based on parameter enable_vins in launch file
