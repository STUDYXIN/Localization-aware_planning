...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg) {
  ...
  takeoff_trigger = ((double)msg.channels[9] - 1000.0) / 1000.0;
  ...
  // 0 判断是否触发自动起飞
  if (takeoff_trigger > GEAR_SHIFT_VALUE)
  {
    is_take_off = true;
    // ROS_WARN("takeoff_trigger: %f, takeoff triggered", takeoff_trigger);
  }
  else 
    is_take_off = false;

  // 1 判断是否要进入悬停模式 以及 是否处于悬停模式
  ...
  // 2 在处于悬停模式的基础上, 判断是否要进入命令模式 以及 是否处于命令模式
  if (is_hover_mode) {
...
// @brief 检查 mode, gear, reboot_cmd 的合理性
