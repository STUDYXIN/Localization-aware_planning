...
bool FeatureManager::solvePoseByPnP(Eigen::Matrix3d &R, Eigen::Vector3d &P,
  ...
  // pnp_succ = solvePnPRansac(pts3D, pts2D, K, D, rvec, t, true, 100, 8.0 / focalLength, 0.99,
  // inliers);
...
void FeatureManager::initFramePoseByPnP(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[],
  ...
  if (frameCnt > 0) {
    ...
    if (solvePoseByPnP(RCam, PCam, pts2D, pts3D)) {
      ...
      // cout << "frameCnt: " << frameCnt <<  " pnp Q " << Q.w() << " " << Q.vec().transpose() <<
      // endl; cout << "frameCnt: " << frameCnt << " pnp P " << Ps[frameCnt].transpose() << endl;
    }
  }
}
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[],
  ...
  for (auto &it_per_id : feature) {
    ...
    if (STEREO && it_per_id.feature_per_frame[0].is_stereo) {
      ...
      printf("stereo %d pts: %f %f %f gt: %f %f %f \n",it_per_id.feature_id, point3d.x(),
      point3d.y(), point3d.z(), ptsGt.x(), ptsGt.y(), ptsGt.z());
      */
      continue;
