...
void FusionFunctions::initialize(int _width, int _height, float _fx, float _fy, float _cx, float _cy, float _fuse_far, float _fuse_near)
{
    image_width = _width;
...
void FusionFunctions::fuse_initialize_map(int reference_frame_index,
                                          cv::Mat &input_image,
                                          cv::Mat &input_depth,
                                          cv::Mat &debug_img,
                                          Eigen::Matrix4f &pose,
...
void FusionFunctions::back_project(const float &u, const float &v, const float &depth, double &x, double &y, double &z)
{
    x = (u - cx) / fx * depth;
    y = (v - cy) / fy * depth;
    z = depth;
...
void FusionFunctions::get_huber_norm(float &nx, float &ny, float &nz, float &nb,
...
void FusionFunctions::fuse_surfels_kernel(int thread_i, int thread_num,
                                          ...
                                          Eigen::Matrix4f inv_pose)
{
    ...
    for (int i = begin_index; i < end_index; i++)
    {
        ...
        float norm_diff_cos = norm_c(0) * superpixel_seeds[sp_index].norm_x + norm_c(1) * superpixel_seeds[sp_index].norm_y + norm_c(2) * superpixel_seeds[sp_index].norm_z;
...
void FusionFunctions::calculate_pixels_norms_kernel(int thread_i, int thread_num)
{
    ...
    for (int row_i = start_row; row_i < end_row; row_i++)
        for (int col_i = 1; col_i < image_width - 1; col_i++)
        {
            ...
            norm_y /= norm_length;
            norm_z /= norm_length;
            float view_angle = (norm_x * my_x + norm_y * my_y + norm_z * my_z) / std::sqrt(my_x * my_x + my_y * my_y + my_z * my_z);
