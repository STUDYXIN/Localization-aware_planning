...
{
  /* map properties */
  Eigen::Vector3d map_origin_, map_size_;               // 全局地图的原点和尺寸
  Eigen::Vector3d map_min_boundary_, map_max_boundary_; // 全局地图的包围盒
  Eigen::Vector3i map_voxel_num_;                       // 全局地图的体素数量
  Eigen::Vector3i map_min_idx_, map_max_idx_;           // 全局地图的包围盒(索引形式)
  Eigen::Vector3d local_update_range_;                  // 局部地图更新范围（尺寸的一半）
  double resolution_, resolution_inv_;                  // 全局地图的分辨率及其倒数
  double obstacles_inflation_;                          // 占据地图膨胀的距离
  string frame_id_;
  int pose_type_; // 位姿输入类型(1: pose_stamped, 2: odometry)
  ...
  /*
   *  p_hit(prob_hit_log_):一次hit增加的概率值
   *  p_miss(prob_miss_log_):一次miss减少的概率值
   *
   *  (clamp_min_log_-unknown_flag_)~clamp_min_log:未知
   *  clamp_min_log~min_occupancy_log:空闲
   *  min_occupancy_log~clamp_max_log:占据
   */
  ...
  double local_bound_inflate_; // ESDF地图边缘扩展的宽度（以米为单位）
  int local_map_margin_;       // 局部地图边缘扩展的宽度（以像素数为单位）
...
struct MappingData
{
  ...
  // 更新ESDF地图并不以当前无人机位置为中心，而是扫描得到障碍物组成的包围盒
...
private:
  ...
  int setCacheOccupancy(const Eigen::Vector3i &idx, int occ);
  ...
  using SynchronizerImagePose = shared_ptr<message_filters::Synchronizer<SyncPolicyImagePose>>;
  using SynchronizerImageOdom = shared_ptr<message_filters::Synchronizer<SyncPolicyImageOdom>>;
