...
void SDFMap::initMap(ros::NodeHandle &nh)
{
  ...
  global_map_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/occupancy_global", 10);
...
void SDFMap::raycastProcess()
{
  ...
  // 更新ESDF地图的范围是根据本轮探测到的深度点云的包围盒决定的
  // 保证当前位置在local_bound_min_和local_bound_max_之间
  ...
  // 这里会根据local_bound_inflate_先膨胀一次
  ...
  while (!md_.cache_voxel_.empty())
  {
    ...
    // 不在局部地图范围内的栅格，直接置为min_log（不是已知，但是是空闲状态的下限）
...
void SDFMap::clearAndInflateLocalMap()
{
  ...
  // local_bound_min_,local_bound_max_代表了内层的边界,如何决定local_bound_min_,local_bound_max_见raycastProcess函数
...
void SDFMap::publishLocalMap(const bool show_margin)
{
  pcl::PointXYZ pt;
  pcl::PointCloud<pcl::PointXYZ> cloud;
  ...
  if (show_margin)
  {
    min_cut -= mp_.local_map_margin_ * Eigen::Vector3i::Ones();
    max_cut += mp_.local_map_margin_ * Eigen::Vector3i::Ones();
  }
