...
namespace fast_planner
{
  ...
  void KinoReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    // 这2s是为了等待visualization初始化完成，不然下面的displayGoalPoint会失效
  ...
  void KinoReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
    {
      ...
      cout << "end_pt_: " << end_pt_.transpose() << endl;
    }
  ...
  void KinoReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case GEN_NEW_TRAJ:
    {
      ...
      changeFSMExecState(success ? EXEC_TRAJ : GEN_NEW_TRAJ, "FSM");
    ...
    case REPLAN_TRAJ:
    {
      ...
      changeFSMExecState(callKinodynamicReplan() ? EXEC_TRAJ : GEN_NEW_TRAJ, "FSM");
  ...
  void KinoReplanFSM::checkSafetyCallback(const ros::TimerEvent &e)
  {
    ...
    if (have_target_)
    {
      ...
      double dist = planner_manager_->pp_.dynamic_ ? edt_env->evaluateCoarseEDT(end_pt_, info->duration_) : edt_env->evaluateCoarseEDT(end_pt_, -1.0);
        ...
        for (double r = dr; r <= 5 * dr + 1e-3; r += dr)
        {
          for (double theta = -90; theta <= 270; theta += dtheta)
          {
            for (double nz = 1 * dz; nz >= -1 * dz; nz -= dz)
            {
              ...
              dist = planner_manager_->pp_.dynamic_ ? edt_env->evaluateCoarseEDT(new_pt, /* time to program start+ */ info->duration_) : edt_env->evaluateCoarseEDT(new_pt, -1.0);
  ...
  {
    ros::Time t_replan = ros::Time::now();
    if (planner_manager_->kinodynamicReplan(start_pt_, start_vel_, start_acc_, end_pt_, end_vel_))
    {
      planner_manager_->planYaw(start_yaw_);

      ROS_WARN("[Local Planner] Plan local motion time: %fs", (ros::Time::now() - t_replan).toSec());
