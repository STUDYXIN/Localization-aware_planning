...
namespace fast_planner
{
  ...
  bool FastPlannerManager::topoReplan(bool collide)
  {
    ...
    if (!collide)
    { // simply truncate the segment and do nothing
    ...
    else
    {
      ...
      else
      {
        ...
        global_data_.setLocalTraj(local_data_.position_traj_, t_now, local_traj_duration + time_inc + t_now, time_inc);
      }
    }
    updateTrajInfo();
    return true;
                                               ...
                                               vector<Eigen::Vector3d> guide_path, int traj_id)
  {
    ...
    Eigen::MatrixXd opt_ctrl_pts2 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(opt_ctrl_pts1, dt, BsplineOptimizer::NORMAL_PHASE, 1, 1);
