...
mat jacobianF(const colvec &X, const colvec &U, double dt)
{
  ...
  F(0, 1) = 0;
  F(0, 2) = 0;
  F(0, 3) = (dt * dt * (ay * (sin(ro) * sin(ya) + cos(ro) * cos(ya) * sin(pt)) + az * (cos(ro) * sin(ya) - cos(ya) * sin(pt) * sin(ro)))) /
            2,
       F(0, 4) = (dt * dt * (az * cos(pt) * cos(ro) * cos(ya) - ax * cos(ya) * sin(pt) + ay * cos(pt) * cos(ya) * sin(ro))) /
                 2;
  F(0, 5) =
  ...
  F(1, 1) = 1;
  F(1, 2) = 0;
  F(1, 3) = -(dt * dt * (ay * (cos(ya) * sin(ro) - cos(ro) * sin(pt) * sin(ya)) + az * (cos(ro) * cos(ya) + sin(pt) * sin(ro) * sin(ya)))) /
            2;
  F(1, 4) = (dt * dt * (az * cos(pt) * cos(ro) * sin(ya) - ax * sin(pt) * sin(ya) + ay * cos(pt) * sin(ro) * sin(ya))) /
            2;
  F(1, 5) =
